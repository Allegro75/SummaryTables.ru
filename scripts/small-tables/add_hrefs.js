...
const addHref = (node, year, tourney) => {
    node.innerHTML = `
                            ...
                            target="_blank"
                            title="Посмотреть турнир ${tourney[1]} чемпионов ${year - 1}/${year}">
                                ${node.innerHTML}
                            </a>                            
                        `;
};
...
for (let i = 1; i < table.rows.length; i ++) {
    ...
    (table.rows[i].cells[4].textContent.includes(`чемпионов`)) ) {
        ...
        if (+table.rows[i].cells[3].textContent >= 1995) {
            ...
            table.rows[i].cells[5].textContent === `отбор`) {
                for (let ci = 2; ci <= 5; ci ++) {
                    addHref(table.rows[i].cells[ci], +table.rows[i].cells[3].textContent + 1, ['CL', 'лиги']);
                }
            }
            else if (table.rows[i].cells[5].textContent === `группа2`) {
                let year = defineYearForGroup2Stage(i);
                for (let ci = 2; ci <= 5; ci ++) {
                    addHref(table.rows[i].cells[ci], year, ['CL', 'лиги']);
                }                
            }
            else {
                for (let ci = 2; ci <= 5; ci ++) {
                    addHref(table.rows[i].cells[ci], +table.rows[i].cells[3].textContent, ['CL', 'лиги']);
                } 
            }
        }
        ...
        // Если Рапид Вена когда-нибудь будет добавлен, то надо будет менять скрипт.
            ...
            (table.rows[i].cells[5].textContent === `1/4`) ) {
                for (let ci = 2; ci <= 5; ci ++) {
                    addHref(table.rows[i].cells[ci], +table.rows[i].cells[3].textContent, ['CC', 'кубка']);
                }                 
            }
            else {
                for (let ci = 2; ci <= 5; ci ++) {
                    addHref(table.rows[i].cells[ci], +table.rows[i].cells[3].textContent + 1, ['CC', 'кубка']);
                }                
            }
        }        
    }
