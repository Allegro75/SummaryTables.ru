...
function getClubByName($name, $clubs)
{
    for ($i = 0; $i < count($clubs); $i++) {
        // Делаем массив альтернативных имён:
        $altNamesArr = [];
        if ($clubs[$i]['altNames']) {
            $altNamesArr = explode(',', $clubs[$i]['altNames']);
        }
        // if ( ($clubs[$i]["basicFullName"] === $name) || ( strpos($clubs[$i]['altNames'], $name) !== false ) ) {
        if ( ($clubs[$i]["basicFullName"] === $name) || (in_array($name, $altNamesArr)) ) {
            return $clubs[$i];
        }
    }
}
    ...
    for ($i = 0; $i < count($pointsArr) - 1; $i++) {
        ...
        if ($pointsArr[$i][6] == $pointsArr[$i + 1][6]) {
            ...
            if ($pointsArr[$i + 1][6] > $pointsArr[$i + 2][6]) {
                // и при этом у нас однокруговой турнир:
                if ( ($pointsArr[$i][1] + $pointsArr[$i][2] + $pointsArr[$i][3]) < count($pointsArr) ) {
            ...
            // Если же у нас тройной делёж:
                ...
                // и при этом у нас однокруговой турнир:
                if ( ($pointsArr[$i][1] + $pointsArr[$i][2] + $pointsArr[$i][3]) < count($pointsArr) ) {
                    $threeClubsPointsArr = [$pointsArr[$i], $pointsArr[$i + 1], $pointsArr[$i + 2]];
                    // Сортируем этот тройственный массив по разности мячей:
                    usort($threeClubsPointsArr, 'compareByGoalsDiff');
                    // echo "<pre>";
                    // print_r($threeClubsPointsArr);                    
                    $resultArr[$i] = $threeClubsPointsArr[0];
                    $resultArr[$i + 1] = $threeClubsPointsArr[1];
                    $resultArr[$i + 2] = $threeClubsPointsArr[2];
                    $i++;
                } 
                // А если турнир двухкруговой:
                else {                                
...
// Функция для получения ДАННЫХ и СОРТИРОВКИ команд в ГРУППЕ:
    // Служебная функция для использования при сортировке (сортирует по очкам):
...
// Функция для заполнения содержания ГРУППОВОй стадии:
function writeGroupStage($orderedClubs, $matches, $stage, $clubs, $imagesList, $tourneyYear) {
    ...
    // Перебираем все элементы нашего массива orderedClubs (состоящего из команд, вышедших из групп) -
    // - участников плей-офф - и таким образом, внутри каждой из итераций перебора мы описываем полностью одну группу):

    $target = count($orderedClubs);
    ...
    for ($i = 0, $num = 1; $i < $target; $i++) {
        ...
        // Если матчей не нашлось (а это актуально для случая команд, пришедших из ЛЧ),
        // то переходим к следующему клубу из плей-офф:
        if (count($leaderMatches) === 0) {
            continue;
        }
        ...
        $thisGroupMatches = array_merge( $thisGroupMatches, getPairMatches($leaderRivalNames[1], $leaderRivalNames[4], $groupMatches) );
        $thisGroupMatches = array_merge( $thisGroupMatches, getPairMatches($leaderRivalNames[2], $leaderRivalNames[4], $groupMatches) );
        $thisGroupMatches = array_merge( $thisGroupMatches, getPairMatches($leaderRivalNames[3], $leaderRivalNames[4], $groupMatches) );
        // echo '<pre>';
        // print_r($thisGroupMatches);
        ...
        $logo5 = getImageAdress(getClubByName($groupInfo[4][0], $clubs), $imagesList);
                ...
                <table class='group-table'>
                    <thead>
                        ...
                        if ($groupInfo[4]) {
                            echo 
                            "<td>
                                <img class='football-logo-table {$logo5[1]}' src='../../images/{$logo5[0]}' alt='{$groupInfo[4][0]}' title='{$logo5[2]}'>
                            </td>"; 
                            }                             
                    ...
                    for ($ri = 0, $number = 1; $ri < count($groupInfo); $ri++, $number++) {
                        ...
                        <tr>
                            ...
                            for ($index = 0; $index < count($groupInfo); $index++) {
                                ...
                                // Ячейка для заполнения собственным лого внутри таблицы:
                                if ($index === $ri) {
                                    echo
                                    "<td>
                                        <img class='football-logo-table {$logo[1]}' src='../../images/{$logo[0]}' alt='{$groupInfo[$ri][0]}' title='{$logo[2]}'>
                                ...
                                // Если матча не было (для однокруговых групп):
                                else if (!$match) {
                                    echo
                                    "<td class='match-cell'>
                                        <div class='group_score'></div>
                                ...
                                // И наконец нормальный случай (матч был):
                                else {
                                    echo
        ...
        // Дополняем наш массив упорядоченных клубов: 
            // Переписываем тут функцию из ЛЧ под нужды ЛЕ:
        if ($tourneyYear <= 2009) {
            $ind = 3;
        } else {
            $ind = 2;
