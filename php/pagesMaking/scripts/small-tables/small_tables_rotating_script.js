...
//
// Функция для определения правильной формы слова "победа" в зависимости от числа этих побед:
const victInRus = (num) => {
    if (num === 1) return `победа`;
    else if ((num !== 0) && (num <= 4)) return `победы`;
    else return `побед`;
}
// Функция для определения правильной формы слова "поражение" в зависимости от числа этих поражений:
const lesionsInRus = (num) => {
    if (num === 1) return `поражение`;
    else if ((num !== 0) && (num <= 4)) return `поражения`;
    else return `поражений`;
}
...
paragraphs[1].textContent = `${oldDefeats} ${victInRus(oldDefeats)}${resultsPText.slice(resFirstCommaInd, resSecCommaInd + 1)} ${oldVictories} ${lesionsInRus(oldVictories)}`;
...
<span class="duels-score">${oldDuelsDefeats}</span> ${victInRus(oldDuelsDefeats)} в дуэлях, 
<span class="duels-score">${oldDuelsVictories}</span> ${lesionsInRus(oldDuelsVictories)}
`;
//
